NB. Learning objects for Catalan video lab in jconsole
cocurrent 'cat'
...
htmpack=: 3 :'''<head><meta charset="UTF-8">'', (CSS rplc ''<FONT>'';''courier''),''</head><body>'', y ,''</body>'''

jhtml=: 4 : 0
'w h'=: > {: x
(htmpack '<svg width="',(": w),'" height="',(": h),'" >',y,'</svg>') 1!:2 < jpath '~addons/media/videolabs/catcon.html'
launch jpath '~addons/media/videolabs/catcon.html'   
)
jhtml2=: 3 : 0
(htmpack y) 1!:2 < jpath '~addons/media/videolabs/catcon.html'
launch jpath '~addons/media/videolabs/catcon.html'   
)
...
tm=.'<text  x="10" y="20" style="font-size:20px">', ,&'</text>' ;(<'<tspan x="0" dy="35">'),"1 ,.&(<'</tspan>') x (('<tspan class="br">R</tspan>';'<tspan class="bb">B</tspan>') {~ >@:cat)y
...
(i. catalan y) path y
:
if. y=0 do. jhtml2 '<svg height="12" width="12"><rect x="1" y="1" width="10" height="10" stroke="grey" fill="grey" /></svg>' return. end. 
if. y=1 do. t=. '<svg height="14" width="14"><rect x="2" y="2" width="10" height="10" stroke="grey" fill="none" />'
            ...
            jhtml2 t,'<path stroke="forestgreen" stroke-width="3" fill="none" d="M 1,13 v -11 h 11" /></svg>' return. end.
...
jhtml2"1 (w , ,&'</svg>') , pos paths >(x (<. <:@:catalan) y) cat y
)
